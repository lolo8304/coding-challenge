...
public class IncrCommand extends SetCommand {
    ...
    private static CommandLine getCommandLine(String key, String value, boolean noreply) {
        return new CommandLine(
                String.format("incr %s %s %s", key, value, noreply ? "noreply" : ""));
    }

    public IncrCommand(CommandLine commandLine) {
        super(commandLine, null);
    }

    public IncrCommand(String key, String value) {
        super(getCommandLine(key, value), null);
    }

    public IncrCommand(String key, String value, boolean noreply) {
    ...
    public Optional<ValidationCode> setToContext(CacheContext cacheContext) {
        ...
        try {
            var incrValue = this.parameter0();
            if (incrValue.isPresent()) {
                cacheContext.command().incr(incrValue.get());
